 (function() {
     ...
     // Tryb testowy — wejście z ?test=1 (ten sam parametr, którego strona już używa,
     // żeby wyłączyć Cloudflare Analytics) nie zapisuje NIC do arkusza. Dzięki temu
     // Twoje własne wizyty/klikanie po stronie w celu sprawdzenia czegoś nie zaśmiecają
     // statystyk w dashboardzie.
     const isTestMode = getQueryParam('test') === '1';
     if (isTestMode) console.log('[AzzurroFunnel] tryb testowy — zdarzenia NIE są zapisywane do backendu');
     ...
     function trackStage(stageName, details) {
         ...
         if (isTestMode) return; // tryb testowy — nie wysyłaj nic do backendu
